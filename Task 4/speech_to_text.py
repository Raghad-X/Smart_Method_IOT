...
def read_audio(ws, timeout):
    ...
    time.sleep(1)
    f= open("written text.txt","a",encoding='utf-8')
    if ('results' in FINALS[0]):
        f.write(FINALS[0]['results'][0]['alternatives'][0]['transcript']+"\n")
        f.close()
        process(FINALS[0]['results'][0]['alternatives'][0]['transcript'])
    ws.close()
